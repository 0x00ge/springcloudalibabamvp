...
import { createUser, deleteUser, fetchUserPageConfig, fetchUsers, updateUser } from '@/api/user.ts'
...
// 弹窗标题根据 editingId 自动切换，避免维护两套弹窗结构。
...
// 3. 当前项目由 MockJS 拦截 /api/users 并返回动态数据。
...
<template>
  <section class="page-view">
    ...
    <el-card v-loading="pageLoading" class="table-card" shadow="never">
      ...
      <div class="toolbar">
        <el-input v-model="keyword" class="search-input" placeholder="搜索用户名称、角色或邮箱" clearable />
