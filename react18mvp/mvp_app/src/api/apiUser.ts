...
import type { UserForm, UserInfo, UserInfoConfig, UserQuery } from '@/types/userTypes'
...
const toUserInfo = (user: UserDto): UserInfo => ({
  id: user.id || '',
  name: user.name,
  phone: user.phone,
...
const toUserDto = (data: UserForm): UserDto => ({
  name: data.name,
  phone: data.phone,
...
export const getUserInfoConfig = async (): Promise<UserInfoConfig> => ({
  roleOptions: [
    { label: '管理员', value: '管理员', tagType: tagTypeMap.warning },
    { label: '普通用户', value: '普通用户', tagType: tagTypeMap.info },
  ],
  statusOptions: [
    { label: '正常', value: '正常', tagType: tagTypeMap.success },
    { label: '禁用', value: '禁用', tagType: tagTypeMap.info },
    { label: '注销', value: '注销', tagType: tagTypeMap.danger },
  ],
  defaultUserForm: {
    name: '',
    phone: '',
...
export const selectUsers = async (query?: Partial<UserQuery>) => {
...
export const deleteUser = (id: string) => del<void>('/user/' + id)
